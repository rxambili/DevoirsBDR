...
package Combat1 {
  ...
  class node(val id: Int, val name: String, val hp: Int = 1, val armor: Int, val regen: Int, val atk: Int, val damages: String, val death: Boolean = false) extends Serializable {
  ...
  class Combat1 extends Serializable {
    ...
    def sendHpValue(ctx: EdgeContext[node, String, node]): Unit = {
      if (ctx.srcAttr.death == false && ctx.dstAttr.death == false) {
    ...
    def takeDamages(vid: VertexId, sommet: node, totalDamages: Int): node = {
      ...
      return new node(sommet.id, sommet.name, newHp, sommet.armor, sommet.regen, sommet.atk, sommet.damages, death = newHp <= 0)
    }
    ...
    def execute(g: Graph[node, String], maxIterations: Int, sc: SparkContext): Graph[node, String] = {
      ...
      def loop1: Unit = {
        while (true) {
          ...
          var printedMessages = messages.collect()
          ...
          myGraph = myGraph.mapVertices(
            (vid, sommet) => new node(sommet.id, sommet.name, sommet.hp + sommet.regen, sommet.armor, sommet.regen, sommet.atk, sommet.damages, sommet.death))
  ...
  object testCombat1 extends App {
    ...
    var myVertices = sc.makeRDD(Array(
      (1L, new node(id = 1, name = "Solar", hp = 363, armor = 44, regen = 15, atk = 35, damages = "3d6 + 18", death = false)),
